...
def get_formatted_dataset(df, min_magnitude=0., select_cols=["time", "longitude", "latitude", "magnitude", "place"]):
    """
...
def get_cell_pos_and_center(lon, lat, bbox=BBOX, n_cells_hor=N_CELLS_HOR, n_cells_ver=N_CELLS_VER):
    LEFT_BORDER = bbox[0]
    RIGHT_BORDER = bbox[1]
    DOWN_BORDER = bbox[2]
    UP_BORDER = bbox[3]
    ...
    if not in_bbox:
        ...
        x = int((x-LEFT_BORDER) / cell_size_hor)
        y = int((y-DOWN_BORDER) / cell_size_ver)

        x_center = x*cell_size_hor + 0.5*cell_size_hor + LEFT_BORDER
        y_center = y*cell_size_ver + 0.5*cell_size_ver + DOWN_BORDER
